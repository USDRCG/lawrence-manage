...
default_email_list = ['bill.conn@usd.edu']
...
def build_output(hour_data, year, month, total_hours):
    ...
    # build totals
    output += "---------------------------------------" + os.linesep
    output += "Total Node Hours for " + str(year) + "/" + str(month).zfill(2) + ":   " + str(round(total_hours, 2)) + os.linesep
    return output
...
def main():
    ...
    if not yes_or_no("Email results?"):
