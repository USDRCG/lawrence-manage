...
for u in "${LOWERCASE_USERS[@]}"; do
	...
	if $(user_in_slurm $local_user) ; then
		#ALL_OK=false
		echo $local_user already in slurm group "$WORKSHOP_GROUP"
	fi
done

# Check if all users are setup
if [ "$ALL_OK" = false ]; then
	echo "These users are not in slurm yet, set them up, then rerun this script"
	echo "Exiting..."
	exit -1
fi
...
for u in "${LOWERCASE_USERS[@]}"; do
	local_user=$(replace_domain $u)
	CMD="sacctmgr -i add user ${local_user} account=${WORKSHOP_ACCOUNT} fairshare=parent"
	$CMD
done
